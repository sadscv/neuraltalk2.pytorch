#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 17-9-14 下午12:47
# @Author  : sadscv
# @File    : prepro_labels_mod.py
"""
...
def encode_captions(imgs, params, wtoi):
    ...
    caption_counter = 0
    counter = 1
    # Todo read prepro_labels and draw mindmap about the label_h5 file
    for i, img in enumerate(imgs):
        n = len(img['final_captions'])
        assert n > 0, 'error: some image has no captions'

        #对于每张图的5个caption，每个caption（sentence)的每个word,都将其转为word_idx.
        #每张图的word_idx存在对应的Li中，Li[j,k]代表第j个caption的第k个word,
        #
